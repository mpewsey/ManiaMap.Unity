            ...
            LayerMask cellLayer, LayerMask triggeringLayer)
        {
            switch (room.RoomType)
            {
                case RoomType.TwoDimensional:
                    return CellArea2D.InstantiateCellArea2D(row, column, room, cellLayer, triggeringLayer);
                case RoomType.ThreeDimensionalXY:
                case RoomType.ThreeDimensionalXZ:
                    return CellArea3D.InstantiateCellArea3D(row, column, room, cellLayer, triggeringLayer);
                default:
